...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  (require 'exwm-randr)
  (setq exwm-randr-workspace-output-plist '(0 "eDP1" 1 "DP2-2" 3 "DP2-2"))
  (add-hook 'exwm-randr-screen-change-hook
	    (lambda ()
	      (start-process-shell-command
	       "xrandr" nil "xrandr --output eDP1 --mode 2048x1152; xrandr --output eDP1 --mode 2048x1152 --right-of DP2-2 --auto")))
  (exwm-randr-enable)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  (defun jrm/wake-second-monitor ()
    ...
    (interactive)
    (shell-command "xrandr --output DP2-2 --off && xrandr --output DP2-2 --auto"))
...
(defun jrm/start-vlc-video ()
  "Start a video in VLC that can be controlled via emacs"
  (interactive)
  (let ((x (read-file-name "Enter video to play:"))
	(videoBuffer "video-controller"))
    (get-buffer-create videoBuffer)
    (shell videoBuffer)
    (process-send-string videoBuffer (concat "vlc -I rc " x "\n"))
    (message "Now playing %s." x)))

(defun jrm/pause-vlc-video ()
  "Pause a running vlc stream"
  (interactive)
  (let ((videoBuffer "video-controller"))
    (process-send-string videoBuffer "pause\n")))

(defun jrm/resume-vlc-video ()
  "Play/resume a running vlc stream"
  (interactive)
  (let ((videoBuffer "video-controller"))
    (process-send-string videoBuffer "play\n")))

(defun jrm/quit-vlc-video ()
