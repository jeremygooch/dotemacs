 ...
 #+BEGIN_SRC emacs-lisp  :tangle ~/.emacs
  ...
  (use-package flycheck :diminish flycheck-mode)
   ...
   (custom-set-variables
    ...
    '(tide-node-executable "/home/jrm/.nvm/versions/node/v9.11.1/bin/node"))
...
#+begin_src emacs-lisp :tangle ~/.emacs
  (defun jrm/narrow-neon ()
    "Hide code based on JS/TS syntax for standard imports of multiple modules"
    (interactive)
    (let ((final-location (point)))
      (if (equal major-mode 'typescript-mode)
	  (funcall (lambda () "Use regex to hide the imports"
      (end-of-buffer)
      (search-backward-regexp "from[^\(].*;")
      (next-line)
	...
	(overlay-put new-overlay 'category "narrow-neon"))

      (goto-char final-location)
      (pop-mark)
      (message "Imports hidden"))))))

  (defun jrm/widen-neon ()
    "Widen and recenter buffer"
    (interactive)
    (if (equal major-mode 'typescript-mode)
	(mapcar
	 'delete-overlay
	 (delq nil
	       (mapcar (lambda (overlay)
			 (and (member "narrow-neon" (overlay-properties overlay)) overlay))
		       (overlays-in (point-min) (point-max)))))))


  (global-set-key (kbd "C-x C-j C-n") 'jrm/narrow-neon)
  (global-set-key (kbd "C-x C-j C-w") 'jrm/widen-neon)
