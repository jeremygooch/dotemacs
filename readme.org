...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  ...
  (when (or (fboundp 'js-mode) (fboundp 'typescript-mode))
    (defun my-ecmascript-config ()
      ...
      (local-set-key (kbd "C-x C-j C-l") 'log-variable)
      ;;
      )
  ...
  (when (or (fboundp 'js-mode) (fboundp 'typescript-mode))
    (defun my-ecmascript-config ()
      "For use in js/ts mode-hooks"
      (local-set-key (kbd "C-x C-j l") 'console-log)
      ;;
      )
    (add-hook 'js-mode-hook 'my-ecmascript-config)
    (add-hook 'typescript-mode-hook 'my-ecmascript-config))
...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  ...
  (defun jrm/start-node-server ()
    "Kill previous running node process and start new node server as a process from current buffer."
    (interactive)
    (setq kill-buffer-query-functions nil) ; dont prompt for killing buffer with running process
    (if (get-buffer "node-shell") (kill-buffer "node-shell"))

    (let ((previousBuffer (current-buffer)))
      (get-buffer-create "node-shell")
      (switch-to-buffer "node-shell")
      (shell (current-buffer))
      (process-send-string "node-shell" "cd ~/neon/; node --inspect dist/server;\n")
      (switch-to-buffer previousBuffer)))

  (defun jrm/neon-refresh ()
    "Refreshes the browser after running some neon tasks."
    (interactive)
    (jrm/start-node-server)
    (browser-refresh))

  (global-set-key (kbd "s-r") 'jrm/refresh)
...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  ...
  (defun jrm/reload-quil-repl ()
    ...
    (let ((previousBuffer (current-buffer)))
      ...
      (switch-to-buffer previousBuffer)))
  (global-set-key (kbd "C-x C-j q")  'jrm/reload-quil-repl)
