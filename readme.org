...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  ...
  (dolist (hook '(org-mode-hook
		  ...
		  shell-mode-hook
		  help-mode-hook))
    (add-hook hook 'linum-off-mode-setup))
...
#+begin_src emacs-lisp :tangle ~/.emacs
  ...
  (add-hook 'ng2-ts-mode-hook 'jrm/neon-toggle-imports)
  (add-hook 'typescript-mode-hook 'jrm/neon-toggle-imports)
...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs 
  (defun jrm/stack-imports ()
	...
	(beginning-of-line)
	(let ((regStart (point)))
	  (while (and (>= regEnd (point)) (re-search-forward "," nil t))
	    ;; the cursor will have moved during the test so check again that we haven't overflowed
	    (if (>= regEnd (point))
		(replace-match ",
  ")))
	  (goto-char regStart)
  ...
  (global-set-key (kbd "C-x C-j C-n") (lambda () (interactive) (jrm/stack-imports) (jrm/neon-toggle-imports)))
