...
#+begin_src emacs-lisp :tangle ~/.emacs
  ...
  (defun jrm/tide-format-before-save ()
    "Use tide and tslint to fix buffer after saving"
    (condition-case nil (tide-organize-imports) (error nil)) ;; times out frequently so trap errors

    (async-shell-command (concat "~/src/cb-consume-ui/node_modules/tslint/bin/tslint --fix " (expand-file-name buffer-file-name))))
  (add-hook 'before-save-hook 'jrm/tide-format-before-save)
