...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  (defvar jrm/screens-alist '((?1 "X11" (lambda () (set-face-attribute 'default nil :height 110) 'default))
			      (?2 "MacOS" (lambda () (set-face-attribute 'default nil :height 120) 'proj))
			      (?3 "Projector" (lambda () (set-face-attribute 'default nil :height 140) 'proj))
			      (?4 "Projector - Large" (lambda () (set-face-attribute 'default nil :height 160) 'projLg)))
  ...
  Returns whatever the action returns."
    (interactive)
    (let ((choice (read-char-choice (mapconcat (lambda (item) (format "%c: %s" (car item) (cadr item))) jrm3/screens-alist "; ")
				    (mapcar #'car valeriy-alist))))
      (funcall (nth 2 (assoc choice jrm3/screens-alist)))))
...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  ...
  ;; "xrandr" nil "xrandr --output eDP-1 --mode 1920x1080")))
  "xrandr" nil "xrandr --output eDP-1 --mode 2048x1152")))
  (exwm-randr-enable)
#+END_SRC
...
A bit of helpful typsecript debugging
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  (fset 'log-variable
	(lambda (&optional arg) "Keyboard macro." (interactive "p") (kmacro-exec-ring-item (quote ("\355console.log('', );\370kmac" 0 "%d")) arg)))

  (when (or (fboundp 'js-mode) (fboundp 'typescript-mode))
    (defun my-ecmascript-config ()
      "For use in js/ts mode-hooks"
      (local-set-key (kbd "C-x C-j l") 'log-variable)
      ;;
      )
    (add-hook 'js-mode-hook 'my-ecmascript-config)
    (add-hook 'typescript-mode-hook 'my-ecmascript-config))
#+END_SRC
...
****** Angular/TS Development
 ...
 #+BEGIN_SRC emacs-lisp  :tangle ~/.emacs
  ...
  (custom-set-variables
   '(flycheck-typescript-tslint-executable "/Users/jgooch/.nvm/versions/node/v10.8.0/bin/tslint"))
