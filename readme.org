...
#+begin_src emacs-lisp :tangle ~/.emacs
  (use-package rjsx-mode
    ...
    (add-to-list 'auto-mode-alist '("src/elfeed-web-react/.*\\.tsx\\'" . typescript-mode)))
#+end_src
...
***** VTerm
For some reason this isn't currently installing via melpa. 
Ensure that =cmake= is installed.
      #+begin_src emacs-lisp
	(setq libvterm-directory "/tmp/emacs-libvterm")
	(defun jrm/build-emacs-libvterm ()
	  "Build emacs-libvterm"
	  (message "here!")
	  (jrm/async-callback (concat "cd " libvterm-directory " && mkdir -p build && cd build && cmake .. && make") (message "Donezo")))

	;; (defun jrm/build-emacs-libvterm ()
	;;   "Build emacs-libvterm"
	;;   (jrm/async-callback (concat "cd " libvterm-directory " && mkdir -p build && cd build && cmake .. && make") (progn (message "done")))))

	;; (defun jrm/clone-emacs-libvterm ()
	;; ""
	;; (magit))
	(defun jrm/clone-emacs-libvterm ()
	  "Clone emacs-libvterm from github"
	  ;; (jrm/async-callback (concat "git clone https://github.com/akermu/emacs-libvterm.git" libvterm-directory))

	  ;; (magit-clone-regular "https://github.com/akermu/emacs-libvterm.git" libvterm-directory nil)
	  )
	...
	(if (not (file-exists-p libvterm-directory))
	    (progn
	      (message "beginning....")
	      (jrm/async-callback (concat "git clone https://github.com/akermu/emacs-libvterm.git " libvterm-directory) (message "step 1 done"))

	      ;; (jrm/clone-emacs-libvterm)


	      ;; (advice-add (magit-clone-regular "https://github.com/akermu/emacs-libvterm.git" libvterm-directory nil) :after 'yy)
	      ;; (advice-add 'xx :after 'yy)
	      ;; (advice-add 'jrm/clone-emacs-libvterm :after 'jrm/build-emacs-libvterm)
	      ;; (message "yo!")
	      ;; (magit-clone-regular "https://github.com/akermu/emacs-libvterm.git" libvterm-directory nil)
	      ;; (jrm/async-callback (concat "cd " libvterm-directory " && mkdir -p build && cd build && cmake .. && make") (progn (message "done")))
	      ))
...
#+begin_src emacs-lisp :tangle ~/.emacs
  (defun jrm/set-org-table-column-widths ()
    ...
    (interactive)
    (unless (equal (format "%s" major-mode) "org-mode") (error "Not in an org mode file"))
    (beginning-of-line)
...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
  (defvar jrm/screens-alist '((?0 "xsmall" (lambda () (set-face-attribute 'default nil :height 70) 'default))
                              ...
                              (?7 "rediculous" (lambda () (set-face-attribute 'default nil :height 250) 'projLg)))
